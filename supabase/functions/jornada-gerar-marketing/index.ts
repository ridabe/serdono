// Ser Dono — Edge Function "jornada-gerar-marketing" (Jornada Empreendedora,
// Fase 10: Marketing, SDD-43).
//
// Gera, numa chamada só: bio pronta pra colar no Instagram/Facebook/TikTok,
// 5 sugestões de post e 3 sugestões de anúncio — usando o que a jornada já
// sabe sobre o negócio (nome, slogan, nicho, persona e proposta de valor já
// gerados na Fase 2), sem pedir nada de novo ao usuário.
//
// Modelo avançado (RN-10: geração de entregável final, mesmo caso de
// jornada-gerar-documentos) — é conteúdo que o empreendedor vai publicar de
// verdade, não uma explicação curta.
//
// Pode ser chamada quantas vezes o usuário quiser (pedido explícito do dono
// do produto) — cada chamada sobrescreve o conteúdo anterior via upsert,
...
interface MarketingConteudo {
  bio: string;
  posts: { titulo: string; legenda: string }[];
  anuncios: { titulo: string; corpo: string; cta: string }[];
}
...
async function gerarConteudo(contexto: Record<string, unknown>): Promise<MarketingConteudo> {
  const system = [
    ...
    "1 bio curta (até 150 caracteres, pronta pra colar em Instagram/Facebook/TikTok, sem hashtag),",
    "5 sugestões de post (cada uma com um título curto de referência e uma legenda pronta pra usar,",
    "em português simples e no tom da marca) e 3 sugestões de anúncio (cada uma com título, corpo curto",
    "e uma chamada pra ação/CTA). Nunca invente promoção, preço ou desconto que não esteja no contexto —",
    "sugestões de anúncio ficam genéricas o suficiente pro empreendedor completar com a oferta real dele.",
    "Responda EXCLUSIVAMENTE com um objeto JSON válido, sem markdown, sem texto antes ou depois, no formato:",
    '{"bio":string,"posts":[{"titulo":string,"legenda":string}],"anuncios":[{"titulo":string,"corpo":string,"cta":string}]}',
  ].join(" ");
  ...
  try {
    return extractJson(text) as MarketingConteudo;
  } catch {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!instance || !instance.nome_empresa_escolhido) {
      return new Response(JSON.stringify({ error: "Escolha o nome da empresa antes de gerar o conteúdo de marketing" }), {
        status: 404,
        headers: { ...corsHeaders, "content-type": "application/json" },
      });
    }
