...
4. Habilitar "Anonymous sign-ins" no projeto Supabase (Authentication → Settings) — bloqueia o diagnóstico funcionar de ponta a ponta antes do cadastro. Ver SDD-15.
5. Aplicar `supabase/migrations/*.sql` no projeto real (`klvmbytlqnvydjsauigy`) e fazer deploy da Edge Function `diagnostic-match` com o secret `ANTHROPIC_API_KEY` — nenhuma das duas foi possível a partir deste ambiente (sem acesso MCP ao projeto). Ver SDD-16.
...
**SDD-16 (acesso ao projeto Supabase real):** este ambiente de desenvolvimento não teve acesso MCP ao projeto Supabase `klvmbytlqnvydjsauigy` (só a outros dois projetos de organizações diferentes ficaram visíveis). As migrations em `supabase/migrations/` e a Edge Function em `supabase/functions/diagnostic-match/` foram escritas como arquivo (fonte de verdade, SPEC §4.1) mas **não aplicadas/deployadas** — isso precisa ser feito via `supabase link` + `supabase db push` + `supabase functions deploy diagnostic-match` (com `supabase secrets set ANTHROPIC_API_KEY=...`), ou reconectando o MCP do Supabase ao projeto certo.
